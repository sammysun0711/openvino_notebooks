...
from transformers import AutoTokenizer, CLIPImageProcessor, CLIPTextModel, CLIPTokenizer, CLIPVisionModelWithProjection
...
def get_pipeline_components(
    ...
    if use_lcm:
        adapter = MotionAdapter.from_pretrained(lora_adapter_path, trust_remote_code=True)
        pipe = AnimateDiffPipeline.from_pretrained(model_id, motion_adapter=adapter, trust_remote_code=True)
        pipe.scheduler = LCMScheduler.from_config(pipe.scheduler.config, beta_schedule="linear", trust_remote_code=True)

        pipe.load_lora_weights(lora_adapter_path, weight_name="AnimateLCM_sd15_t2v_lora.safetensors", adapter_name="lcm-lora", trust_remote_code=True)
        pipe.load_lora_weights(motion_adapter_path, adapter_name="tilt-up", trust_remote_code=True)

        pipe.set_adapters(["lcm-lora", "tilt-up"], [1.0, 0.8])

    else:
        adapter = MotionAdapter.from_pretrained(motion_adapter_path, trust_remote_code=True)
        # load SD 1.5 based finetuned model
        pipe = AnimateDiffPipeline.from_pretrained(model_id, motion_adapter=adapter, **pipe_kwargs)
...
def convert_animatediff(
    model_id="emilianJR/epiCRealism",
    motion_adaper_path="guoyww/animatediff-motion-lora-tilt-up",
    ...
    if requires_conversion:
        unet, vae, text_encoder = (
            get_pipeline_components(model_id, motion_adaper_path, lora_adapter_path, use_lcm)
        )
    else:
...
class OVAnimateDiffPipeline(
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if clip_skip is None:
                ...
                #prompt_embeds = torch.from_numpy(self.text_encoder(text_input_ids.to(device), attention_mask=attention_mask))
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            """
            negative_prompt_embeds = torch.from_numpy(self.text_encoder(uncond_input.input_ids)[0])
    ...
    def decode_latents(self, latents, decode_chunk_size: int = 16):
        ...
        #(FIX me) need to checkout this part!
        ...
        #print("height: ", height)
        #print("width: ", width)
        #print("latents before permute shape: ", latents.shape)
        #print("latents before permute: ", latents)
        latents = latents.permute(0, 2, 1, 3, 4).reshape(batch_size * num_frames, channels, height, width)
        #print("latents after permute shape: ", latents.shape)
        #print("latents after permute: ", latents)

        video = []
        for i in range(0, latents.shape[0], decode_chunk_size):
            batch_latents = latents[i : i + decode_chunk_size]
            #print("batch_latents.shape: ", batch_latents.shape)
            #batch_latents = self.vae.decode(batch_latents).sample
            #print("batch_latents before vae: ", batch_latents)
            batch_latents = torch.from_numpy(self.vae(batch_latents)[0])
            #print("batch_latents after vae: ", batch_latents)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #print("self.free_noise_enabled: ", self.free_noise_enabled)
        if self.free_noise_enabled:
        ...
        for free_init_iter in range(num_free_init_iters):
            ...
            with self.progress_bar(total=self._num_timesteps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    noise_pred = torch.from_numpy(self.unet(unet_inputs)[0])
        ...
        if output_type == "latent":
            ...
            #print("--------------------------------- Vae decoder inputs ----------------------------")
            #print("latents.shape: ", latents.shape)
            #print("decode_chunk_size: ", decode_chunk_size)
            #print("latents to decode : ", latents)
            video_tensor = self.decode_latents(latents, decode_chunk_size)
            #print("video tensor: ", video_tensor)
...
def init_animatediff_pipeline(
    ...
    for model_name, model_path in models_dict.items():
        pipeline_args[model_name] = core.compile_model(model_path, device, ov_config)
        print(f"{model_name} - Done!")

    tokenizer = AutoTokenizer.from_pretrained(MODEL_DIR / "tokenizer")

    pipeline_args["tokenizer"] = tokenizer
    ov_pipe = OVAnimateDiffPipeline(**pipeline_args)

    if use_lcm: 
        ov_pipe.scheduler = LCMScheduler.from_config(MODEL_DIR / "scheduler", beta_schedule="linear")
    else:
        ov_pipe.scheduler = DDIMScheduler.from_pretrained(
        MODEL_DIR,
        subfolder="scheduler",
        clip_sample=False,
        timestep_spacing="linspace",
...
if __name__ == "__main__":
    ...
    if use_lcm:
        ...
        motion_adapter_path="animatediff-motion-adapter-v1-5-2"
    ...
    if use_lcm:
        ...
        export_to_gif(frames, "animatelcm_ov_lcm.gif")
